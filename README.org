...
Given a transducer that may error, wrap it in =restart-case= to provide various
ways of skipping or handling that error.
...
If the elements are all either 0 or 1, you can reduce into a =bit-vector= instead:
...
Collect a stream of characters into a single string. The result is the ideal
...
However, if you know your source characters are all ASCII (rather, the =base-char=
range for your particular compiler), then =base-string= often has much lower
memory consumption:
